...
const fetchPost = async () => {
    try {
        ...
        // Chuyển đổi 'created-at' thành định dạng YYYY-MM-DD
        if (post["created-at"]) {
            const timestamp = post["created-at"] * 1000;
            const parsedDate = new Date(timestamp);
            if (!isNaN(parsedDate)) {
                date.value = parsedDate.toISOString().split("T")[0];
            } else {
                console.error("Invalid timestamp:", post["created-at"]);
            }
        } else {
            console.warn("No 'created-at' provided in API response.");
        }
        ...
        selectedType.value =
            types.value.find((t) => t.name === post.type) || null;
    } catch (error) {
...
const handleUpdatePost = async () => {
    ...
    const timestamp = new Date(date.value).getTime() / 1000;
    ...
    const formdata = {
        ...
        "created-at": timestamp, // Sử dụng timestamp cho `created-at`
    };
...
<template>
    <div class="content">
        ...
        <form @submit.prevent="handleUpdatePost">
            ...
            <BaseButton btnTitle="Update Post" variant="primary" />
        </form>
    </div>
</template>
